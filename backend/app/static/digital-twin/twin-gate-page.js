(function () {
  ...
  function noSelection() {
    current = null;
    contextHost.innerHTML = ui.stateView("empty", "No execution context selected", "Choose a mock twin to inspect its immutable Twin Gate.", false);
    gateHost.innerHTML = ui.stateView("empty", "No Digital Twin selected", "Open Bundle Execution from a Digital Twin row or choose a mock twin below.", false) + '<div class="variant-switcher">' + ["green", "amber", "red", "stale", "expired", "generating"].map(function (variant) { return '<button class="btn" type="button" data-gate-fixture="' + variant + '">' + ui.label(variant) + "</button>"; }).join("") + "</div>";
