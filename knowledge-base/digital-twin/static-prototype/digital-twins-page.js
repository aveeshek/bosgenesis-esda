(function () {
  ...
  function renderResponse(response) {
    ...
    if (!response.items.length) {
      ...
      stateHost.innerHTML = ui.stateView("empty", hasFilter ? "No matching twins" : "No Digital Twins yet", hasFilter ? "Clear one or more filters to restore the full browser fixture list." : "Generate the first browser-only mock twin.", false);
    } else {
      stateHost.hidden = true;
      body.innerHTML = response.items.map(rowMarkup).join("");
    }
    summary.textContent = response.page.result_count + " results · fixture contract " + response.schema_version;
  ...
  document.addEventListener("click", function (event) {
    ...
    if (actionButton) {
      ...
      if (code === "open_twin") window.location.href = ui.detailHref(twinId, "overview");
      if (code === "open_execution") window.location.href = "bundle-execution-twin-gate.html?twin_id=" + encodeURIComponent(twinId);
      ...
      if (code === "regenerate") adapter.regenerate(twinId).then(function (next) { window.location.href = ui.detailHref(next.twin_id, "overview", { progress: "1" }); });
    ...
    if (row) window.location.href = ui.detailHref(row.getAttribute("data-twin-row"), "overview");
  ...
  document.addEventListener("keydown", function (event) {
    ...
    if (row && (event.key === "Enter" || event.key === " ")) {
      ...
      window.location.href = ui.detailHref(row.getAttribute("data-twin-row"), "overview");
    }
  });
  ...
  document.getElementById("generate-fixture").addEventListener("click", function () {
    adapter.startGeneration(scenarioSelect.value).then(function (twin) {
      window.location.href = ui.detailHref(twin.twin_id, "overview", { progress: "1" });
    });
  });

  document.getElementById("clear-mock-history").addEventListener("click", function () {
    adapter.clearMockHistory().then(function () { ui.showToast("Browser-only mock history cleared.", "success"); load(); });
  });
